    ...
    <div class="content-wrapper">
        <section class="content-header top-header" style="margin-left: 0px; margin-right: 0px;">
            <div class="row">
                <div class="col-md-6">
                    <h2>
                        {{ __('Contact Sector of Activities') }} / @if ($action == 'Add') {{ __('Add') }} @else {{ __('Edit') }} @endif
                    </h2>
                </div>
                <div class="col-md-6">
                    ...
                    <div class="col-xs-12">
                        <div class="box box-success box-solid">
                            <div class="box-header with-border">
                                <h3 class="box-title">@if ($action == 'Add') {{ __('Add New Contact Sector Activity') }} @else {{ __('Edit Contact Sector Activity') }} @endif</h3>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <form class="timmunity-custom-dashboard-form mt-2 form-validate"
                                            action="{{ route('admin.contacts-sectors-activities.store') }}" method="post">
                                            @csrf
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="description">{{ __('Description') }}<small class="asterik" style="color:red">*</small></label>
                                                    <input type="text"
                                                        class="form-control @error('description') is-invalid @enderror"
                                                        id="description" name="description"
                                                        value="{{ old('description', translation(@$model->id, 6, app()->getLocale(), 'description', @$model->description) ?? '') }}"
                                                        maxlength="255" aria-describedby="description" required />
                                                    @error('description')
                                                        <div id="description-error"
                                                            class="invalid-feedback animated fadeInDown">{{ $message }}
                                                        </div>
                                                    @enderror
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                ...
                                                <a style="border-bottom: 2px solid #009a71;" class=" btn ml-2"
                                                    href="{{ route('admin.contacts-sectors-activities.index') }}">{{ __('Discard') }}</a>
                                            </div>
                                    </div>
                                </div>
                            </div>
