...
@section('content')
    ...
    <div class="content-wrapper">
        <section class="content-header top-header">
            <div class="row">
                ...
                <div class="col-md-6">
                    @if (  (currency_format($model->total_payable* $model->exchange_rate,'','',1)) > $model->amount_paid && $model->refunded_at == null)
                                            ...
                                            <table id="product_order_line_table"
                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><a href="#.">{{ __('Order#') }} <span class="caret"></span></a></th>
                                                        ...
                                                        <th><a href="#.">{{ __('Pending') }}<span class="caret"></span></a></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    ...
                                                    @endphp
                                                        <tr class="order_lnes">
                                                            <td class="product_total">{{$ind++}}</td>
                                                            @if($payment_detail->voucher_order->reseller)
                                                            <td class="product_total">{{ str_replace(' ','',$payment_detail->voucher_order->reseller->name).'-'.str_pad($payment_detail->voucher_order->id,3,'0',STR_PAD_LEFT).'-'.date('dmY',strtotime($payment_detail->voucher_order->created_at)); }}</td>
                                                            @elseif($payment_detail->voucher_order->distributor)
                                                            <td class="product_total">{{ str_replace(' ','',$payment_detail->voucher_order->distributor->name).'-'.str_pad($payment_detail->voucher_order->id,3,'0',STR_PAD_LEFT).'-'.date('dmY',strtotime($payment_detail->voucher_order->created_at)); }}</td>
                                                            @endif
                                                            <td class="product_name">
                                                                {{ $payment_detail->voucher_order->product->product_name.' '.@$payment_detail->voucher_order->variation->variation_name }} voucher payment
                                                            </td>
                                                            <td class="product_description">
                                                                {{ count(explode(',',$payment_detail->voucher_ids)) }}
                                                            </td>
                                                            <td class="product_qty">
                                                                {{ currency_format(( @$payment_detail->voucher_order->unit_price - ( @$payment_detail->voucher_order->unit_price * @$payment_detail->voucher_order->discount_percentage / 100) ) * $model->exchange_rate,$model->currency_symbol,$model->currency) }}
                                                            </td>
                                                            <td class="product_unit_price">
                                                                {{ @$payment_detail->voucher_order->taxes}}
                                                            </td>
                                                            <td class="product_taxes">
                                                            {{ currency_format(@$payment_detail->total_payable* $model->exchange_rate,$model->currency_symbol,$model->currency) }}
                                                            </td>
                                                            <td class="product_taxes">
                                                            {{ currency_format(@$payment_detail->pending_payment,$model->currency_symbol,$model->currency) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            ...
                                            <div class="col-md-4" style="text-align:right">
                                                ...
                                                <p>
                                                    ...
                                                    <span id="taxed_amount">
                                                        {{ currency_format($model->tax_amount* $model->exchange_rate,$model->currency_symbol,$model->currency) }}
                                                    </span>
                                                </p>
                                                <hr>
                                                <p>
                                                    ...
                                                    <span id="total_amount">
                                                        {{ currency_format($model->total_payable* $model->exchange_rate,$model->currency_symbol,$model->currency) }}
                                                    </span>
                                                </p>
                                                <p>
                                                ...
                                                @php
                                                $remaining_amount = currency_format($model->total_payable* $model->exchange_rate,'','',1) - currency_format($model->amount_paid,'','',1);
                                                @endphp
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                @php
                                    $total =  currency_format($model->total_payable* $model->exchange_rate,'','',1);
                                @endphp
                                <label class="control-label" for="registered_amount">{{ __('Amount').' ('.$model->currency.')' }}</label>
    ...
    <div class="modal fade" id="refund-payment" tabindex="-1" role="dialog" aria-hidden="true" >
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="{{ route('admin.voucher.orders.payment.refund', $model->id) }}" method="POST" id="register-payment-form">
                @csrf
                <input type="hidden" name="invoice_id" value="{{ Hashids::encode($model->id) }}">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                        <div class="row">
                            <div class="col-md-6 text-left">
                                <button type="button" class="btn btn-secondary"
                                    data-dismiss="modal">{{ __('Cancel') }}</button>
                                <input type="submit" class="btn btn-primary register-payment-btn-submit" value="Make Refund" Payment/>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            @foreach($model->voucher_payment_references as $ph)
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text"  readonly="readonly" class="form-control" value="{{ currency_format($ph->amount,$model->currency_symbol,$model->currency)  }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="control-label" for="method">{{ __('Payment Method') }}</label>
