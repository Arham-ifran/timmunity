<div class="modal fade" id="send-by-email" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <h4>{{ __('Recipents') }}:</h4>
                                <input type="hidden" name="send_email[type]" value="0">
                            ...
                            <div class="form-group">
                                <h4>{{ __('Subject') }}:</h4>
                                <input class="form-control" type="text" name="send_email[email_subject]" value="@isset($model){{ __('Order Quotation Ref')}}( S{{ str_pad($model->id, 5, '0', STR_PAD_LEFT) }} @endisset )" placeholder="">
                            </div>
                            <br>
                            <div class="form-group">
                                <textarea class="textarea" name="send_email[email_body]"
                                    placeholder="Place some text here">@isset($model){{ __('Hello') }},&nbsp;<strong>{{ $model->customer->name }}</strong><br>{{ __('Your quotation') }} <strong>S{{ str_pad($model->id, 5, '0', STR_PAD_LEFT) }}</strong> {{ __('amounting in') }} <strong>{{ $model->currency_symbol }} {{ currency_format(str_replace(",","",$model->total)* $model->exchange_rate,'','',1) }} {{ $model->currency }}</strong> {{ __('is ready for review.')}}<br>{{ __('Do not hesitate to contact us if you have any questions.') }}@endisset
                                </textarea>
                            </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-xs-2 col-sm-2 col-md-2 pl-0 pr-0 text-center">
                                            {{-- <a target="_blank" href="{{ asset('/storage/quotations/S'.str_pad($model->id, 5, '0', STR_PAD_LEFT).'_inv.pdf' ) }}?download=true" title="S{{ str_pad($model->id, 5, '0', STR_PAD_LEFT) }}.pdf" aria-label="Download"> --}}
                                            <a target="_blank" class="pdf_link_a" href="#." title="    S{{ str_pad($model->id, 5, '0', STR_PAD_LEFT) }}.pdf" aria-label="Download">
                        ...
                        <div class="row">
                            <div class="col-md-6 text-left">
                                ...
                                <button type="button" class="btn btn-primary send-email-btn-submit">{{ __('Send') }}</button>
                            </div>
