   ...
   <!-- Table content -->
    <section class="content kss-subscription-box-sections">
        <div class="box pt-1">
                    ...
                    <div class="row filter_row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="" class="control-label">Expiry Date Range</label>
                                <input type="text" class="form-control" id="end_date_filter" name="end_date_filter" autocomplete="off">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="" class="control-label">License Code</label>
                                <input type="text" id="license_code" class="form-control" autocomplete="off">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
            ...
            <div class="row">
                <table id="KSSLicenseTable" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>{{ __('License') }}</th>
                            <th>{{ __('Customer') }}</th>
...
<script>
    ...
    ajax_data['end_date'] = '';
    ajax_data['expiry_start_date'] = '';
    ajax_data['expiry_end_date'] = '';
    var table = $('#KSSLicenseTable').DataTable({
        ...
        stateSave: true,
        "ajax": {
            "url": "{{route('admin.kss.licenses')}}",
            "data": function(d){
                d.license_code= $('#license_code').val();
                d.customer= $('#customer').val();
                d.type= $('#type').val();
                d.coupon= $('#coupon').val();
                d.start_date= ajax_data['start_date'];
                d.end_date= ajax_data['end_date'];
                d.expiry_start_date= ajax_data['expiry_start_date'];
                d.expiry_end_date= ajax_data['expiry_end_date'];
    ...
    $('#end_date_filter').daterangepicker({
        ...
        console.log('New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')');
        ajax_data['expiry_start_date'] = start.format('YYYY-MM-DD');
        ajax_data['expiry_end_date'] = end.format('YYYY-MM-DD');
        table.ajax.reload();
    });
    $('#end_date_filter').val('Select Expiry Date Range')
</script>
@endsection
